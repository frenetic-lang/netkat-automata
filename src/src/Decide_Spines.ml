...
let rec lrspines (e : term) : TermSet.t =
  ...
  | Times l ->
    ...
    | d :: t ->
      ...
      | Times [l;r] -> Times [l; Decide_Ast.simplify (Times (r :: t))]
      | _ -> failwith "lrspines 1" in
      let r = TermSet.map f u in
      let g x = match x with
      | Times [l;r] -> Times [Decide_Ast.simplify (Times [d;l]); r]
  ...
  | Star d ->
    let s = lrspines d in
    let f x = match x with
    | Times [l;r] -> Times [Decide_Ast.simplify (Times [e;l]); 
			    Decide_Ast.simplify (Times [r;e])]
...
let display_lrspines (ts : TermSet.t) : unit =
  ...
  | Times [l;r] -> Printf.printf "%s ||| %s\n" 
    (Decide_Ast.term_to_string l) (Decide_Ast.term_to_string r)
