
let loop_freedom edge_pol pol topo _ (*out_edge_pol *) = 
  (* version one: take one hop, and use deriv term for rest *)
  let open Decide_Ast in 
  let open Decide_Base in 
  let open Decide_Util in 
  let open Decide_Deriv in 
  if set_univ [Term.values edge_pol; Term.values pol; Term.values topo]
  then 
    begin
      let trm = Term.make_times 
	[edge_pol; 
	 Term.make_star (Term.make_times [pol;topo])] in 
      let dtrm = DerivTerm.make_term trm in 
      let dmat,pset = DerivTerm.run_d dtrm in 
      Base.Set.fold_points 
	(fun pt acc -> 
	  let beta_t = Term.of_complete_test (Base.point_rhs pt) in
	  let alpha_t = Term.of_complete_test (Base.point_lhs pt) in
	  let dtrm_t = DerivTerm.to_term (dmat pt) in 
	  let newterm = 
	    (Term.make_times 
	       [beta_t; dtrm_t ; alpha_t]) in
	  let em = Term.one_dup_e_matrix newterm in 
	  if (Base.Set.is_empty em)
	  then acc
	  else (Printf.printf "Bad! Circular path found: %s\n"
		  (Base.complete_test_to_string (Base.point_lhs pt));
		false
	  )
	) pset true
    end
  else true

let loop_freedom_one = loop_freedom

let loop_freedom edge_pol pol topo _ (*out_edge_pol *) = 
  (* version two: take one non-starred hop, and use starred original term for rest *)
  let open Decide_Ast in 
  let open Decide_Base in 
  let open Decide_Util in 
  let open Decide_Deriv in 
  if set_univ [Term.values edge_pol; Term.values pol; Term.values topo]
  then 
    begin
      let trm = Term.make_times 
	[edge_pol; pol; topo] in 
      let dtrm = DerivTerm.make_term trm in 
      let dmat,pset = DerivTerm.run_d dtrm in 
      Base.Set.fold_points 
	(fun pt acc -> 
	  let beta_t = Term.of_complete_test (Base.point_rhs pt) in
	  let alpha_t = Term.of_complete_test (Base.point_lhs pt) in
	  let newterm = 
	    (Term.make_times 
	       [beta_t; Term.make_star (Term.make_times [pol;topo]) ; alpha_t]) in
	  let em = Term.one_dup_e_matrix newterm in 
	  if (Base.Set.is_empty em)
	  then acc
	  else (Printf.printf "Bad! Circular path found: %s\n"
		  (Base.complete_test_to_string (Base.point_lhs pt));
		false
	  )
	) pset true
    end
  else true
