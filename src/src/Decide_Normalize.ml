...
let universe (f : formula) : S.t =
  let univ = values_in_formula f in
  let g u x = S.add x (-1) (* JNF: "" *) u in
  List.fold_left g univ (S.keys univ)


let ssm_to_string (t : S.t) (op : ('a -> 'b -> 'c, unit, string) format) : string =
  let s = S.bindings t in
  let f (x,a) = Printf.sprintf op (string_of_int x) (String.concat "," (List.map string_of_int a)) in
...
let forall_atoms (f : ssm_pair -> bool) ((t,p) : ssm_pair) : bool =
  ...
  let rec forall (c : S.t) (s : (id * (int list)) list) : bool =
    match s with
      | [] -> f (c,p)
